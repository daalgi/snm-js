const { areEqual, sum } = require("./utils")
...
class Matrix {
    ...
    equalSize(other) {
        return this.rows_number === other.rows_number &&
    ...
    submatrixRemoving(row, column) {
        return new Matrix(...this.rows.filter((_, i) =>
            i != row).map(row => row.filter((_, j) => j != column)))
    }
