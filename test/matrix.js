...
describe("linear-algebra module - Matrix object", () => {
  ...
  describe("Matrix class defines a matrix", () =>{
      ...
      it("Matrix.random({rows: 3}) defines a 3x3 matrix with numbers between 0 and 1", () => {
          let r = Matrix.random({rows: 3});
          assert.equal(r.rows_number, 3);
          assert.equal(r.columns_number, 3);
          for(let i = 0; i < r.rows_number; i++){
              for(let j = 0; j < r.columns_number; j++) {
                  assert.isAtLeast(r.rows[i][j], 0);
                  assert.isAtMost(r.rows[i][j], 1);
              }
          }
      });
      it("Matrix.random({rows: 3, min: 8, max: 13}) defines a 3x3 matrix with numbers between 8 and 13", () => {
          let r = Matrix.random({rows: 3, min: 8, max: 13});
          assert.equal(r.rows_number, 3);
          assert.equal(r.columns_number, 3);
          for(let i = 0; i < r.rows_number; i++){
              for(let j = 0; j < r.columns_number; j++) {
                  assert.isAtLeast(r.rows[i][j], 8);
                  assert.isAtMost(r.rows[i][j], 13);
              }
          }
      });
      ...
      describe("Matrix([1, 2, 3], [2, 1, 5], [-1, 2, -1])", () => {
          ...
          describe("multiplication of two matrices 3x4 and 4x3", () => {
              it("should return a Matrix of 3x3", () => {
                  let m34 = Matrix.random({rows: 3, cols: 4});
                  let m43 = Matrix.random({rows: 4, cols: 3});
                  let res = m34.multiply(m43);
                  assert.equal(res.rows_number, 3);
                  assert.equal(res.columns_number, 3);
              });
          });
